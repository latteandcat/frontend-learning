...
- 访问元素：一般通过括号 `arr[i]` 访问元素，等同于 `arr.at(i)`
...
- `filter()`：将运行该函数返回 true 的项组成的数组返回
...
- `toString(base)`：将数组转成字符串，并且按照 base 进制进行转化
  - base 的范围为2~36，默认为10
...
- `charAt(pos)`：获取字符串某个位置的字符等同于 `str[pos]`

  `str[pos]` 没有找到会返回 undefined

  而 `charAt(pos)` 没有找到会返回空字符串
